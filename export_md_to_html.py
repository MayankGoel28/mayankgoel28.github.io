...
# Clean, minimal batch convert all .md files in md_files/ to .html in md_files/
import os
import re
...
def convert_obsidian_images(md_text):
    def obsidian_img(match):
        inner = match.group(1)
        parts = inner.split('|')
        src = parts[0].strip()
        alt = parts[1].strip() if len(parts) > 1 else os.path.splitext(os.path.basename(src))[0]
        return f'<img src="{src}" alt="{alt}" style="border-radius:4px;max-width:900px;">'
    return re.sub(r'!\[\[(.*?)\]\]', obsidian_img, md_text)

def remove_show_tag(md_text):
    return re.sub(r'(?<!\\)\\show(?![a-zA-Z])', '', md_text)

def add_title(md_text, title):
    return f'# {title}\n\n' + md_text.lstrip()
    ...
    try:
        ...
        for line in md_text.splitlines():
            if line.startswith('#'):
                ...
                html_lines.append(f'<h{level}>{html.escape(line.lstrip("# "))}</h{level}>')
            else:
...
def process_md_file(md_path):
    ...
    with open(md_path, 'r', encoding='utf-8') as f:
        md_text = f.read()
    md_text = remove_show_tag(md_text)
    md_text = convert_obsidian_images(md_text)
    md_text = add_title(md_text, title)
